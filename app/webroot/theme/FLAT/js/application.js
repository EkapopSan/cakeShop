...
function initialNavigatorState() {
    $.cookie.json = true;
    if ($.cookie('nav-state') != undefined) {
        var navState = $.cookie('nav-state');
        $('.subnav').each(function (index, div) {
            $(this).prop('class', navState[index]);
        });
    }
}
...
function saveNavigatorState() {
    var navStateObj = {};
    $('.subnav').each(function (index, div) {
        navStateObj[index] = div.className;
    });
    $.cookie('nav-state', navStateObj);
}
